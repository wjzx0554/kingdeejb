// ==UserScript==
// @name         金蝶开发字典助手 V18（可配置模板）
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  polling.do 字典抓取模板：目录/详情/关系自动分类并支持 CSV/JSON 导出
// @match        *://*.jdy.com/*
// @grant        none
...
(function () {
  ...
  const CONFIG = {
    storageKey: 'KD_DEV_DICT_V18',
    version: '18.0',
    maxHits: 120,
    saveDelayMs: 250,
    pollingUrlPattern: /polling\.do/i,
    ui: {
      id: 'kd-dev-ui-v18',
      title: '金蝶开发字典助手 V18',
      position: { left: '20px', bottom: '20px' }
    },
    backendNamePattern: /^(t_|fct_|fs_|dim_|dwd_|ads_|ods_|tmp_|mid_)/i,
    rootNamePattern: /^root(\u0001.*)?$/,
    debug: false
  };
  ...
  const getOfficialDict = () => window.KD_OFFICIAL || { officialTables: [] };

  function defaultStore() {
    return {
      meta: { version: CONFIG.version, updatedAt: '' },
  ...
  function log(...args) {
    if (CONFIG.debug) console.log('[KD-V18-TPL]', ...args);
  }
  ...
  function saveStoreDebounced() {
    ...
    saveTimer = setTimeout(() => {
      ...
      localStorage.setItem(CONFIG.storageKey, JSON.stringify(memStore));
  ...
  const isTechBackendName = (name = '') => CONFIG.backendNamePattern.test(String(name || ''));
  const looksRootCategoryName = (name = '') => CONFIG.rootNamePattern.test(String(name || ''));
  ...
  function walkNode(node, visitor, seen = new WeakSet()) {
    ...
    if (Array.isArray(node)) {
      node.forEach((item) => walkNode(item, visitor, seen));
    } else {
      Object.values(node).forEach((v) => walkNode(v, visitor, seen));
    }
  }
  ...
  function dedupePushHit(hit) {
    ...
    memStore.hits = memStore.hits.slice(0, CONFIG.maxHits);
  }

  function addHit(meta = {}, preview = '') {
    dedupePushHit({
      ts: now(),
      source: meta.source || '',
      url: meta.url || '',
  ...
  function classifyDetailRoot(root) {
    ...
    let chineseAliasCount = 0;
    let englishAliasEqNameCount = 0;
    let fPrefixCount = 0;
    let dottedAssociateCount = 0;
    let fkCount = 0;
    ...
    for (const f of fields) {
      ...
      if (hasChinese(alias)) chineseAliasCount += 1;
      if (alias && alias === fname && !hasChinese(alias)) englishAliasEqNameCount += 1;
      if (/^f[a-z0-9_]+$/i.test(fname)) fPrefixCount += 1;
      if (assoc.includes('.')) dottedAssociateCount += 1;
      if (f?.foreignKey) fkCount += 1;
    }
  ...
  function upsertCatalog(side, item, parentName = '', meta = {}, judgeReason = '') {
    ...
    if (!memStore.catalogs[side][key]) {
      memStore.catalogs[side][key] = { type: item.type || '', name: item.name || '', displayName: item.displayName || item.name || '', commentInfo: item.commentInfo || '', parentName, firstSeen: now(), lastSeen: now(), sourceUrl: meta.url || '', judgeReason };
    } else {
      const o = memStore.catalogs[side][key];
      o.displayName ||= item.displayName || item.name || '';
      o.commentInfo ||= item.commentInfo || '';
      o.parentName ||= parentName;
      o.lastSeen = now();
      o.sourceUrl ||= meta.url || '';
      o.judgeReason ||= judgeReason;
    }
  }
  ...
  function ensureObject(side, name, ent = {}, root = {}, meta = {}, judgeReason = '') {
    ...
    if (!memStore.objects[side][name]) {
      memStore.objects[side][name] = {
        name,
        label: ent.alias || root.displayName || root.alias || root.name || name,
        associateName: ent.associateName || root.associateName || '',
        parentName: ent.parentName || root.parentName || '',
        commentInfo: ent.commentInfo || root.commentInfo || '',
        source: ent.source || root.source || '',
        directModel: !!(ent.directModel ?? root.directModel),
        firstSeen: now(),
        lastSeen: now(),
        sourceUrl: meta.url || '',
        objectType: 'unknown',
      ...
      const o = memStore.objects[side][name];
      o.label ||= ent.alias || root.displayName || root.alias || root.name || name;
      o.associateName ||= ent.associateName || root.associateName || '';
      o.parentName ||= ent.parentName || root.parentName || '';
      o.commentInfo ||= ent.commentInfo || root.commentInfo || '';
      o.source ||= ent.source || root.source || '';
      o.lastSeen = now();
      o.sourceUrl ||= meta.url || '';
      o.judgeReason ||= judgeReason;
    }
  ...
  function tagObjectType(name, fieldsObj) {
    ...
    if (/_entry\b/i.test(lower) || /\bentry\b/i.test(lower)) return 'entry_table';
    if (/_lk\b/i.test(lower)) return 'link_table';
    if (/_l\b/i.test(lower)) return 'lang_table';
    if (/_c\b/i.test(lower)) return 'ext_table';
    ...
    if (fieldNames.some((f) => /^fbill(no|date|type|status)$/i.test(f)) || fieldNames.some((f) => /^fcustomerid$/i.test(f)) || fieldNames.some((f) => /^fmaterialid$/i.test(f))) return 'business_table';
    return 'unknown';
  }
  ...
  function upsertRelation(rel, meta = {}) {
    ...
    if (!memStore.relations[side][key]) {
      ...
      const o = memStore.relations[side][key];
      o.lastSeen = now();
      o.sourceUrl ||= meta.url || '';
      o.judgeReason ||= cls.reason;
    }
  }
  ...
  function processDataRoot(root, meta = {}) {
    ...
    if (Array.isArray(root.entities)) {
      ...
      root.entities.forEach((ent) => {
        ...
        const fields = ent.properties || ent.fields || [];
        fields.forEach((f) => upsertField(side, name, f));
  ...
  function safeConsume(payload, meta = {}) {
    try {
      if (meta.url && !CONFIG.pollingUrlPattern.test(meta.url)) return;
      const parsed = typeof payload === 'string' ? tryParseJSON(payload) : payload;
      if (!parsed || typeof parsed !== 'object') return;

      walkNode(parsed, (node) => {
        if (node?.data?.data && typeof node.data.data === 'object') processDataRoot(node.data.data, meta);
        if (Array.isArray(node?.children) || Array.isArray(node?.entities) || Array.isArray(node?.relations)) processDataRoot(node, meta);
      });
    } catch (e) {
      console.warn('[KD-V18-TPL] consume failed:', e);
    }
  }
    ...
    this.addEventListener('load', function () {
      try {
        if (typeof this.responseText === 'string') {
          safeConsume(this.responseText, { source: 'xhr', url: this.__kd_url || '' });
        }
      } catch (e) {
        log('xhr consume error', e);
      }
    });
  ...
  function renderUI() {
    if (uiReady || window.self !== window.top) return;
    ...
    ui.id = CONFIG.ui.id;
    ui.style.cssText = `position:fixed; bottom:${CONFIG.ui.position.bottom}; left:${CONFIG.ui.position.left}; z-index:999999; width:300px; background:#fff; border:2px solid #E91E63; border-radius:10px; padding:12px; box-shadow:0 6px 16px rgba(0,0,0,.18); font-size:12px; color:#333;`;

    ui.innerHTML = `
      <div style="font-weight:bold;color:#E91E63;margin-bottom:10px;">${CONFIG.ui.title}</div>
